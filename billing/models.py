...
# СИСТЕМА ОПЛАТЫ
import stripe
stripe.api_key = "sk_test_51JOQm5Faw5MMBhhdWTgtpl7aQMmxYoKGBfkK55Im9ePD7a0iL73SIEOmG80ghCP8Bb68273CRcvx6LdKlSMh1v2u00mjfLJdUG"
...
class BillingProfile(models.Model):
    ...
    def default_card(self):
        default_qs = self.get_cards().filter(active=True, default=True)
        if default_qs.exists():   
            return default_qs.first()
        return None
    
def pre_save_customer_create_receiver(sender, instance, *args, **kwargs):
    if not instance.customer_id and instance.email:
        customer = stripe.Customer.create(
            email = instance.email
        )
        instance.customer_id = customer.id
pre_save.connect(pre_save_customer_create_receiver, sender=BillingProfile)
...
def set_default_card_receiver(sender, instance, created, *args, **kwargs):
    if instance.default:
        billing_profile = instance.billing_profile
        qs = Card.objects.filter(billing_profile=billing_profile).exclude(pk=instance.id)
        qs.update(default=False)
post_save.connect(set_default_card_receiver, sender=Card)
...
class Charge(models.Model):
    ...
    outcome         = models.TextField(max_length=120, blank=True, null=True)
    ...
    def __str__(self):
        return f"{self.billing_profile} {self.paid}"
